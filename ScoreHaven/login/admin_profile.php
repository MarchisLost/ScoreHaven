...
<?php
	...
	//Mudar idade
    if (isset($_POST['change_age_save_btn'])) {
    	$new_age = $_POST["new_age"];
    	$select_a = mysqli_query($ligacao, "SELECT `idade` FROM `users` WHERE `idade` = '$new_age'") or exit(mysqli_error($ligacao));

        if(mysqli_num_rows($select_a)) {
?>
        	<div class="container">
              <div class="alert alert-warning">
                <strong>Warning!</strong> The age cannot be the same as the previous one
              </div>
            </div>
<?php
		}
	    else{
			$sql_age_change = "UPDATE users SET idade='$new_age' WHERE id_u='$user_id'";
    		mysqli_query($ligacao, $sql_age_change);
    		
?>
			<script type="text/javascript">
				$("#change_age_save_btn").click(function() {
					$("#change_age_form").hide();
					$("#change_age_btn").show();
				});			
			</script>
			...
			<div class="container">
	            <div class="alert alert-success">
	              <strong>Success!</strong> Age changed successfully! Relog to see changes
	            </div>
	        </div>
<?php
	    }
...
?>
                            ...
                            <div class="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                <button type="button" id="change_age_btn" class="btn btn-primary btn-sm" data-toggle="collapse" data-target="#change_age_form">Change</button>
                                                <div class="collapse" id="change_age_form">
                                                    <form method="post">
                                                        <p>
                                                          <input type="text" pattern="^[0-9]*$" class="form-control" required="required" name="new_age" placeholder="Enter new age" />
                                                          <br>  
                                                            <button type="submit" name="change_age_save_btn" id="change_age_save_btn" class="btn btn-success btn-sm">
                                                                <span class="glyphicon glyphicon-ok"></span> Save 
                                                            </button>
                                                            <input class="btn btn-default btn-sm" id="change_age_cancel_btn" type="reset" value="Cancel">
                                                            <span class="glyphicon glyphicon-remove"></span>
                                                        </p>                                                 
                                                    </form>
                                                </div>
...
<script type="text/javascript">
    ...
    $("#change_age_btn").click(function() {
        $("#change_age_btn").hide();
        $("#change_age_form").show();
    });
    
    $("#change_age_cancel_btn").click(function() {
        $("#change_age_form").hide();
        $("#change_age_btn").show();
    });
